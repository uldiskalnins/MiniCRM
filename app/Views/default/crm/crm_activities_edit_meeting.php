  ...
  <div class=" container col-11 text-dark bg-white mt-3 border pl-0 pr-0">
    ...
    <form action="<?= base_url('crm/activities/meetings/save/'); ?>" id="addForm" method="post">
      ...
      <div class="row pl-3 pr-3 pb-3 ">
        ...
        <input type="hidden" id="MeetingId" name="id" value="<?= $meetingData['id'] ?>">
        ...
        <div class="col-sm-4 ">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" name="title" id="title" value="<?= $meetingData['title'] ?>" required>
          </div>

          <div class="form-group">
            <label for="description" class="small"><?= lang('Crm.description') ?></label>
            <textarea class="form-control" id="description" rows="3" name="description"><?= $meetingData['description'] ?></textarea>
          </div>
        ...
        <div class="col-sm-4 ">
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="status" name="status">
              <?php
              echo '<option value="0" ' . ($meetingData['status'] == 0 ? "selected" : "") . '>' . lang('Crm.statusList')[0] . '</option>';
              echo '<option value="1" ' . ($meetingData['status'] == 1 ? "selected" : "") . '>' . lang('Crm.statusList')[1] . '</option>';
              echo '<option value="2" ' . ($meetingData['status'] == 2 ? "selected" : "") . '>' . lang('Crm.statusList')[2] . '</option>';
              ?>
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <input class="form-control" id="startDate" type="text" name="startdate" autocomplete="off" value="<?= $meetingData['start_date'] ?>" required>
          </div>


          <div class="form-group">
            <label for="endDate" class="small"><?= lang('Crm.endDateTime') ?> *</label>
            <input class="form-control" id="endDate" type="text" name="enddate" autocomplete="off" value="<?= $meetingData['end_date'] ?>" required>
          </div>
        ...
        <div class="col-sm-4 ">
          ...
          <div class="form-group">
            ...
            ?>
              ...
              <?php if (!empty($meetingData['user_id'])) : // if associated user exists 
              ?>

                <input type="hidden" id="userId" name="userid" value="<?= $meetingData['user_id'] ?>">


                <div class="input-group  mb-0">
                  <input id="findUser" name="userfullname" type="text" class="form-control" value="<?= $meetingData['user_full_name'] ?> " autocomplete="off">
          ...
          <input type="hidden" id="parentId" name="parentid" value="<?= $meetingData['parent_id'] ?>">
            ...
            <select id="parentType" class="form-control" name="parenttype">
              ...
              if (is_array($allowedParents)) {
                foreach ($allowedParents as $row) {
                  echo ' <option value="' . $row->id . '"' . ($meetingData['parent_type'] == $row->id ? " selected" : "") . '>' . lang('Crm.allParentTypes')[$row->id] . '</option>';
                }
              }
              ?>
          ...
          <div class="form-group">
            ...
            <div class="input-group  mb-0">
              <input id="findParent" type="text" class="form-control" autocomplete="off" value="<?= $meetingData['parent_title'] ?>">
              <div class="input-group-append">
