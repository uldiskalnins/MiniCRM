...
<?= service('validation')->listErrors('my_list'); ?>
 ...
 <form action="<?= base_url('admin/edit-user').'/'.$user['id'] ?>" id="saveForm" method="post">
 <?= csrf_field() ?>
  ...
  <div class="form-group">
   <label for="active"><?= lang('Crm.active')?>:</label>
   <select class="form-control" id="active" name="active" required>
<?php

    if($user['active'] == 1):
        echo '
        <option value="1" selected>'.lang('Crm.yes').'</option>
        <option value="0">'.lang('Crm.no').'</option>
        ';
    else:
        echo '
        <option value="1">'.lang('Crm.yes').'</option>
        <option value="0" selected>'.lang('Crm.no').'</option>
        ';
    endif;
...
<?php
    ...
    while($x < count(lang('Crm.rightsLevelsWords')) ){
        ...
        if (lang("Crm.rightsLevelsNumbers.$x") == $user['user_rights']){
            echo '<option value="'.lang("Crm.rightsLevelsNumbers.$x").'" selected>'.lang("Crm.rightsLevelsWords.$x")."</option>
            ";
        }
        else{
...
?>
  ...
  <button type="submit" id="sbutton" class="btn btn-info float-right"><?= lang('Crm.save');?></button>
...
<script>
    ...
    function togglePassword() {//parāda vai slēpj paroles lauciņa tekstu
